 ...
 */
package io.vertx.rocketmq.client.consumer.rebalance;
    ...
    public List<MessageQueue> allocate(String consumerGroup, String currentCID, List<MessageQueue> mqAll,
        ...
        for (MessageQueue mq : mqAll) {
            ...
            if (StringUtils.isNoneEmpty(brokerMachineRoom)) {
                mr2Mq.computeIfAbsent(brokerMachineRoom, k -> new ArrayList<>());
        ...
        for (String cid : cidAll) {
            ...
            if (StringUtils.isNoneEmpty(consumerMachineRoom)) {
                mr2c.computeIfAbsent(consumerMachineRoom, k -> new ArrayList<>());
        ...
        if (mqInThisMachineRoom != null && !mqInThisMachineRoom.isEmpty()) {
            allocateResults.addAll(allocateMessageQueueStrategy.allocate(consumerGroup, currentCID, mqInThisMachineRoom,
        ...
        for (String machineRoom : mr2Mq.keySet()) {
            if (!mr2c.containsKey(machineRoom)) { // no alive consumer in the corresponding machine room, so all consumers share these queues
                allocateResults.addAll(
                        allocateMessageQueueStrategy.allocate(consumerGroup, currentCID, mr2Mq.get(machineRoom), cidAll));
            }
        }
